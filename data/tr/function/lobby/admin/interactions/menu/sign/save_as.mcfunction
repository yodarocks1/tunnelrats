...
data modify storage tr:tmp new.name set from storage tr:admin edit.name
...
data remove storage tr:settings arenas[-1].builtin
data remove storage tr:settings arenas[-1].info
...
tellraw @p[tag=interactor] [{"color":"green", "translate":"tr.text.admin.arena_saved_as", "with": [{"nbt":"edit.name", "storage": "tr:admin"}]}]
